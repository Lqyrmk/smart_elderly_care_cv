...
class yoloDetect:
    ...
    def __init__(self, url):
        ...
        self.model = attempt_load("models/pt/seepage/best.pt", map_location=self.device)  # 模型文件
                ...
                for i, det in enumerate(pred):
                    ...
                    if len(det):
                        ...
                        if t - self.alert_count_time >= 5:
                            ...
                            # 向数据库插入数据
                            cv_event = Event(event_type=8,
                                             event_date=event_date,
                                             event_location=self.event_location,
                                             event_desc="检测到有房屋渗水，请及时处理",
                                             elderly_id=None)
                            session.add(cv_event)

                            # 提交
                            session.flush()
                            cv2.imwrite('utils/file_cos_utils/img/seepage/' + str(cv_event.event_id) + '.jpg', im0)
                            # 提交
                            session.commit()
                            # 关闭session
                            session.close()
                            ...
                            # 上传到cos
                            # 要上传图片所在的文件夹
                            local_dir_path = 'utils/file_cos_utils/img/seepage'
                            # 图片对应的path，对应上传到的文件夹
                            elderly_path = "/img/" + "seepage"
