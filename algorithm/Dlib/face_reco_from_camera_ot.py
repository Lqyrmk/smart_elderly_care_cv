...
import logging
import torch
from utils.file_cos_utils import cos_loader
...
class Face_Recognizer:
    def __init__(self):
        self.flag = True
        self.hasUnknown = 0
        self.detect_img = cv2.imread("utils/file_cos_utils/img/tip/tip.jpg")
            ...
            while self.flag:
                ...
                if t - self.laugh_count_time >= 10:
                    ...
                    for i, name in enumerate(self.current_frame_face_name_list):
                        ...
                        if role == "elderly" and self.emo[i] == 'happy':
                            ...
                            # 从数据库中查询老人数据
                            elderly_name = self.current_frame_face_name_list[i].split('_')[-1]
                            elderly = session.query(Elderly).filter(Elderly.elderly_name == elderly_name).first()

                            # 向数据库插入事件数据'elderly_2_lym'
                            ...
                            # 提交
                            session.flush()
                            cv2.imwrite('utils/file_cos_utils/img/laugh/' + str(cv_event.event_id) + '.jpg', img_rd)
                            # 提交
                            session.commit()
                            # 关闭session
                            session.close()
                            ...
                            # 上传到cos
                            # 要上传图片所在的文件夹
                            local_dir_path = 'utils/file_cos_utils/img/laugh'
                            # 图片对应的path，对应上传到的文件夹
                            elderly_path = "/img/" + "laugh"
                            # 最终路径为 local_dir_path +
                            cos_loader.upload_file_to_cos(local_dir_path=local_dir_path, elderly_path=elderly_path)
                ...
                if 'unknown' in self.current_frame_face_name_list:
                    ...
                    if t - self.alert_count_time >= 5:
                        ...
                        # 提交
                        session.flush()
                        cv2.imwrite('utils/file_cos_utils/img/unknown/' + str(cv_event.event_id) + '.jpg', img_rd)
                        # 提交
                        session.commit()
                        # 关闭session
                        session.close()
                        ...
                        # 上传到cos
                        # 要上传图片所在的文件夹
                        local_dir_path = 'utils/file_cos_utils/img/unknown'
                        # 图片对应的path，对应上传到的文件夹
                            ...
                            for j in volunteerNum:
                                ...
                                if distance <= 360000:
                                    ...
                                    if t - self.ia_count_time > 10:
                                        ...
                                        elderly_name = self.current_frame_face_name_list[i].split('_')[-1]
                                        volunteer_name = self.current_frame_face_name_list[j].split('_')[-1]
                                        ...
                                        # 从数据库中查询老人数据
                                        elderly = session.query(Elderly).filter(
                                            Elderly.elderly_name == elderly_name).first()

                                        # 向数据库插入数据
                                        cv_event = Event(event_type=4,
                                                         event_date=event_date,
                                                         event_location=self.event_location,
                                                         event_desc="检测到老人[" + elderly_name + "]与义工[" + volunteer_name + "]交互",
                                                         elderly_id=elderly.elderly_id)
                                        session.add(cv_event)
                                        ...
                                        session.flush()
                                        cv2.imwrite(
                                            'utils/file_cos_utils/img/interaction/' + str(cv_event.event_id) + '.jpg',
                                            img_rd)
                                        ...
                                        # 上传到cos
                                        # 要上传图片所在的文件夹
                                        local_dir_path = 'utils/file_cos_utils/img/interaction'
                                        # 图片对应的path，对应上传到的文件夹
                                        elderly_path = "/img/" + "interaction"
                                        cos_loader.upload_file_to_cos(local_dir_path=local_dir_path,
